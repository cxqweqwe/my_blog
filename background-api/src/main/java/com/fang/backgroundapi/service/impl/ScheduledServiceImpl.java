...
public class ScheduledServiceImpl implements ScheduledService {
    ...
    @Value("${com.fang.baseURL}")
    private String baseUrl;

    /**
     * Description: 定时运行，向每一个用户发送邮件
     *
     * @Author: Bernie_fang
    ...
    public void statistics() {
        ...
        for (UserInfo userInfo : userInfos) {
            if (StringUtils.isNotEmpty(userInfo.getEmail().getValue())){
                ...
                String token = JWTUtil.signToToken(userInfo.getAuthorId(), userInfo.getAuthorId(), CommonInfo.THREE_DAY_EXPIRATION_TIME_MILLISECOND);
                String href = baseUrl + "/statistics/show/" + token + "/" +date;
                asyncService.sendEmailStatistics(href, userInfo.getEmail().getValue());
            }
        }
     ...
     * Description: 获取数据
     * @Author: Bernie_fang
     * @Since: 2022/3/3 23:19
     * @param authorId:
     * @param date:
     * @return: java.util.ArrayList<java.util.List<java.lang.Object>>
     **/
    public ArrayList<List<Object>> getData(String authorId, Date date){
        Calendar ca = Calendar.getInstance();           //得到一个Calendar的实例
